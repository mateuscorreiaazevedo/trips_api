import { VerifyPasswordUseCase } from '@core/application/use-cases/authentication'
...
const verifyPasswordUsecase = new VerifyPasswordUseCase(jwtAuthenticationRepository)
...
export async function loginAuthenticationController(req: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const { token } = await verifyPasswordUsecase.execute({ user, password })
